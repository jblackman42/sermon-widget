...
const loadWidget = async () => {
    ...
    sermonContainerDOM.innerHTML = `
        <div id="video-container">
            ...
            <video id="${getFileExtension(videoURL) == 'm3u8' ? 'video' : ''}" class="video-js vjs-default-skin" controls>
                <source src="${videoURL}">
            </video>
        </div>
        <div id="video-card-container">
            <div class="row">
                ...
                <div class="col">
                    <button id="shareBtn" class="btn" onclick="share('Share Sermon')">Share <i class='fas fa-share-square'></i></button>
                    ${audioURL ? `<a href="${audioURL}" class="btn">Audio Download </a>` : ''}
                </div>
            </div>
    ...
    if (Hls.isSupported() && videoURL && !mobile) {
        ...
        var video = document.getElementById("video");
        video.style.backgroundImage = `url('${sermon.Sermon_Image}')`
    ...
    try {
        if (getFileExtension(videoURL) == 'm3u8') {
